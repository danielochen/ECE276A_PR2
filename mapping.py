...
from pr2_itils import bresenham2D, test_bresenham2D, show_lidar, plot_map, lidar_scan_to_points, rotation2d, rotation_yaw, make_T3, make_T2, test_map, encoder_IMU_odometry, Kabsch, ICP, warmup_icp, build_trajectory, ICP_dataset, plot_trajectory, visualize_map
...
def disparity2pointcloud(d_image, rgb_image):
    ...
    mask_d = (z > 0.1) & (z < 30)                                                 # filter out invalid points
...
def rollpitchyaw2R(roll, pitch, yaw):
    Rx = np.array([
        [1, 0, 0],
